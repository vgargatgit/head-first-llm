(() => {
  ...
  const chapters = [
    ...
    makeChapter(10, 'The Residual Stream Climbs the Stack', 'Deep stack', 'How deep blocks refine hidden states and maintain per-layer KV caches.', 'part-2', 2, {
      ...
      artwork: [
        { placement: 'start', src: 'assets/chapter-10/01_the_transformer_tower_explained.png', alt: 'SAT approaches a tower whose floors represent successive Transformer layers.' },
        { afterHeading: 'One block is one refinement step', src: 'assets/chapter-10/02_one_floor_at_a_time_transformer_layer.png', alt: 'One Transformer layer applies self-attention, residual connections, normalisation, and a feed-forward network.' },
        { afterHeading: "SAT's evolving case file", src: 'assets/chapter-10/03_sat_state_evolution_cartoon_diagram.png', alt: 'SAT keeps the same token identity while its hidden representation evolves through the layer stack.' },
        { afterHeading: 'Every layer owns different parameters', src: 'assets/chapter-10/04_layers_and_parameters_explained_visually.png', alt: 'Each Transformer layer has its own separately learned parameters despite sharing the same architecture.' },
        { afterHeading: 'What repeated contextualisation buys the model', src: 'assets/chapter-10/05_context_grows_with_visible_tokens.png', alt: 'SAT gathers information from all visible earlier tokens and develops a richer contextual representation.' },
        ...
        { afterHeading: 'Generation processes one new position at a time', src: 'assets/chapter-10/07_decoding_with_cached_keys_and_values.png', alt: 'Prompt prefill caches Keys and Values, while decoding computes the newest token using the existing cache.' },
        { afterHeading: 'Why model depth costs memory and computation', src: 'assets/chapter-10/08_token_generation_growth_in_transformers.png', alt: 'KV-cache storage grows across both sequence length and Transformer depth.' },
        { afterHeading: 'The final hidden state is still not a token', src: 'assets/chapter-10/09_final_output_path_diagram_with_mascot.png', alt: 'The final hidden state passes through final normalisation and vocabulary projection before becoming token probabilities.' },
